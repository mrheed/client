...
const PDFUlangan = (props) => {
    ...
    const HEADER = ["No", "Nama Lengkap", "Nilai", "Remidi", "Nilai Remidi"]
    ...
    const c = StyleSheet.create({
        ...
        footDetail2: {
            ...styles.tableCol,
            width: (275 - COL1_WIDTH) / (HEADER.length - 1) + "%"
        },
        cellDetail: {
            ...styles.tableCell,
            opacity: 1,
    ...
    return (
        <>
            <PDFViewer width="100%" height={window.innerHeight}>
                <Document title={"Daftar nilai ulangan "+DATA.mapel+" kelas XII KJ"}>
                    <Page style={styles.body} orientation="landscape" >
                        <View style={c.title}><Text>Nilai {
							props.tipe === "UH" 
								? "Ulangan Harian" 
								: props.tipe === "PTS" 
								? "Ulangan Tengah Semester" 
								: "Ulangan Akhir Semester"
						} Siswa SMKN 2 Wonosari</Text></View>
                        <View style={c.dtb}>
						...
						{props.tipe === "UH" && <View style={c.dtb}>
                            <View style={styles.tableRow}>
                                <View style={c.wDesc}><Text>Materi</Text></View>
                                <View style={c.sparator}><Text>:</Text></View>
                                <View style={c.rDesc}><Text>{DATA.materi}</Text></View>
                            </View>
                        </View>}
                        <View style={c.dtb}>
                            <View style={styles.tableRow}>
                        ...
                        <TABLE>
                            <TR>
                                ...
                                <View style={c.thirdH}>
                                    <Text style={styles.tableCellHeader}>Nilai Ulangan</Text>
                                </View>
                                <View style={c.fourthH}>
                                    <Text style={styles.tableCellHeader}>Remidi</Text>
                                </View>
                                <View style={c.fivethH}>
                                    <Text style={styles.tableCellHeader}>Nilai Remidi</Text>
                                </View>
                            </TR>
                            {DATA.data.map((x, i) =>
                                <TR>
                                    ...
                                    <View style={c.third}>
                                        <Text style={styles.tableCell}>{x.nilai_ulangan}</Text>
                                    </View>
                                    <View style={c.fourth}>
                                        <Text style={styles.tableCell}>{x.remidi ? "Ya" : "Tidak"}</Text>
                                    </View>
                                    <View style={c.fiveth}>
                                        <Text style={styles.tableCell}>{x.remidi ? x.nilai_remidi ? x.nilai_remidi : "Belum remidi" : "-"}</Text>
                                    </View>
                                </TR>)}
                            <TR>
                                <TD1><TC></TC></TD1>
